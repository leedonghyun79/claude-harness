...
> 이 파일은 목차만 포함한다. 모든 상세 내용은 docs/에 있다. 백과사전처럼 비대화 금지.

## 작업 시작 전 필수 체크
1. `skills/lessons-learned.md` 읽기 — 등록된 교훈을 코드 작성 전에 반드시 적용
2. `docs/exec-plans/active/` 확인 — 이미 진행 중인 계획이 있는지 체크
3. `scripts/preflight.sh` 실행 — 환경 점검
4. `docs/QUALITY_SCORE.md` 확인 — 현재 품질 기준 파악

## 실행 루프 (모든 작업에 강제)
```
1. 브레인스토밍   →  /brainstorm — 요구사항 정제, 설계 문서 생성
2. 실행 계획      →  /write-plan → docs/exec-plans/active/{작업명}.md 작성
3. 구현           →  슬라이스별 서브에이전트 + TDD (RED → GREEN → REFACTOR)
4. 검증           →  scripts/verify.sh — 통과할 때까지 수정
5. 리뷰           →  /review — 계획 준수 여부 확인
...
| 참고 자료 | [docs/references/](docs/references/) |
...
- **권장**: 보안 스캔, 성능 스모크 테스트
- 모든 PR에 포함 필수: 사용자 영향 · 호환성 · 롤백 절차 · 미해결 리스크
